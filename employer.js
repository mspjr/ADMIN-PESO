document.addEventListener('DOMContentLoaded', function() {
  ...
  async function renumberEstablishments() {
    ...
    if(getEstablishment.success === false) {
        ...
        const tableBody = document.getElementById("establishmentTable").getElementsByTagName('tbody')[0];
        tableBody.innerHTML = "";
        for(i=0; i<getEstablishment.data.length; i++) {
          ...
          row.innerHTML = `
                  ...
                  <td>${new Date(getEstablishment.data[i].createdAt).toISOString().split('T')[0]}</td>
                  ...
                  <td style='display:none;'>${getEstablishment.data[i].email}</td>
                  <td style='display:none;'>${getEstablishment.data[i].contactNumber}</td> 
                  <td style='display:none;'>${getEstablishment.data[i].address}</td> 
                  <td style='display:none;'>${getEstablishment.data[i].establishment_id}</td>     
            `;
        }
    }
  }
  ...
  addEstablishmentForm.addEventListener("submit", async function (e) {
    ...
    const addEstablishmentResult = await addEstablishment(establishmentName, industry, ownerName, address);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (icon.closest('.icon-delete')) {
      ...
      rowToDelete = row;
      const name = row.children[1].textContent;
      document.getElementById("deleteEstablishmentId").value = row.children[10].innerText; // also get establishment_id as reference for db delete
  ...
  document.getElementById('editForm').addEventListener('submit', async (e) => {
    ...
    // editModal.setAttribute('aria-hidden', 'true');
      ...
      const establishment_id =  document.getElementById("editEstablishmentId").value;

      const editEstablishmentResult = await editEstablishment(establishment_id, selectedEstablishmentName, selectedIndustry, selectedOwnerName, selectedAddress);
  ...
  confirmDeleteBtn.onclick = async () => {
    ...
    // deleteOverlay.setAttribute('aria-hidden', 'true');
    // rowToDelete = null;
    const selectedEstablishmentId =  document.getElementById("deleteEstablishmentId").value;
    const deleteEstablishmentResult = await deleteEstablishment(selectedEstablishmentId);
  ...
  async function renumberEmployers() {
    ...
    if(getEmployers.success === false) {
        ...
        //added td for establishment_id but only hidden
        const tableBody = document.getElementById("employersTable").getElementsByTagName('tbody')[0];
        tableBody.innerHTML = "";
        for(i=0; i<getEmployers.data.length; i++) {
          const row = tableBody.insertRow();
          row.innerHTML = `
                  <td>${tableBody.rows.length}</td>
                  <td>${getEmployers.data[i].firstName} ${getEmployers.data[i].middleName ?? ''} ${getEmployers.data[i].lastName ?? ''} ${getEmployers.data[i].suffix ?? ''}</td>
                  <td>${getEmployers.data[i].email}</td>
                  <td>${statusBadge(getEmployers.data[i].status)}</td>
                  <td>${new Date(getEmployers.data[i].created_at).toISOString().split("T")[0]}</td>
                  ...
                  <td style='display:none;'>${getEmployers.data[i].firstName}</td>
                  <td style='display:none;'>${getEmployers.data[i].lastName}</td>
  ...
  document.addEventListener('click', (e) => {
    ...
    if (icon.closest('.icon-edit-emp')) {
      ...
      document.getElementById('editEmpFirst').value  = row.children[7].textContent; //hidden td for first name
      document.getElementById('editEmpLast').value   = row.children[8].textContent; //hidden td for last name
      document.getElementById('editEmpEmail').value  = row.children[2].textContent;
      document.getElementById('editEmpStatus').value = row.children[3].textContent;
      document.getElementById('editEmpUserId').value = row.children[6].textContent;
  ...
  editEmpForm.addEventListener('submit', async (e) => {
    ...
    const editEmployerResult = await editEmployer(userId, first, last, email, stat);
...
async function addEstablishment(establishmentName, industry, ownerName, address)  {
    const { data, error } = await supabase
        .from("Establishment")
        .insert([{ 
...
async function editEstablishment(establishment_id, establishmentName, industry, ownerName, address)  {
