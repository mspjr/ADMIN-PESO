      ...
      for(i=0; i<getBrgyResult.data.length; i++) {
        tbody.insertAdjacentHTML("beforeend", `
          <tr>
              ...
              <td style='display:none;'>${getBrgyResult.data[i].barangay_id}</td> 
            </tr>
            `);
      }
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById("updateBarangayBtn").onclick = async () => {
    ...
    const editBrgyResult = await editBarangay(selectedBarangayId, newBarangayName);
  ...
  document.getElementById("confirmDeleteBtn").onclick = async () => {
    ...
    const selectedBarangayId =  document.getElementById("deleteBarangayId").value;
...
document.addEventListener("click", e => {
  ...
  if (e.target.classList.contains("icon-delete")) {
    ...
    document.getElementById("deleteBarangayId").value = row.children[3].innerText; // also get barangay_id as reference for db delete
